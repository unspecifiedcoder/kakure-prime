...
const scratchDir = resolve(".video-output");
...
await page.goto("https://kakure-prime.vercel.app/?recording=stocklana#/demo", { waitUntil: "networkidle" });
...
if (!recording) throw new Error("Playwright did not produce a recording");
await rename(join(scratchDir, recording), join(outputDir, "kakure-prime-stocklana.webm"));
await rm(scratchDir, { recursive: true, force: true });
